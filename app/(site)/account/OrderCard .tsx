...
export default function OrderCard({ user_id }: OrderCardProps) {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const res = await fetch(`https://bevclock-production.up.railway.app/api/orders?user_id=${user_id}`);
        ...
        await Promise.all(
          data.map(async (order) => {
            const detailRes = await fetch(`https://bevclock-production.up.railway.app/api/order-details/${order._id}`);
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      if (!token) return;
      try {
        const res = await fetch(`https://bevclock-production.up.railway.app/reviews/user`, {
          headers: {
  ...
  const handleCancelOrder = async (order_id: string) => {
    try {
      await fetch(`https://bevclock-production.up.railway.app/api/cancel-order/${order_id}`, {
        method: "PUT",
      });
      setOrders((prev) =>
        prev.map((o) =>
  ...
  try {
    const res = await fetch(`https://bevclock-production.up.railway.app/api/return-order/${order_id}`, {
      method: "PUT",
      headers: {
