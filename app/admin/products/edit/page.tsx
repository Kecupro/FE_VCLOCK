...
const EditProduct = () => {
  ...
  const fetchProduct = async () => {
    try {
      const res = await fetch(`
        https://bevclock-production.up.railway.app/api/admin/product/${productId}`);
      const data = await res.json();
      const p = data;
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`https://bevclock-production.up.railway.app/api/admin/product/sua/${productId}`, {
        method: 'PUT',
        body: formData
      }); 
